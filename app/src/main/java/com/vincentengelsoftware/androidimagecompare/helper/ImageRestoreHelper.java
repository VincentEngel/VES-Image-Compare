...
/**
 * Restores the left/right image slots from persisted URIs and Bundle transform state after
 * a configuration change or process death.
 * <p>
 * The two entry points mirror the two restore phases in {@code Activity.onCreate}:
 * <ol>
 *   <li>{@link #restoreImages} — re-decodes bitmaps from disk when the in-memory holders
 *       are empty (process was killed).</li>
 *   <li>{@link #restoreImageViews} — re-applies already-loaded bitmaps to the preview
 *       {@link android.widget.ImageView}s after every configuration change.</li>
 * </ol>
 */
public class ImageRestoreHelper {
    ...
    /**
     * Loads bitmaps from the stored URIs when the in-memory holders are empty, then
     * updates the preview views for every holder that has a bitmap.
     */
    public static void restoreImages(
        ...
        if (first.getBitmap() == null && sessionState.getLeftImageUri() != null) {
            loadHolder(cr, sessionState.getLeftImageUri(), first,
                    cacheDir, CacheManager.COMPARE_FILE_ONE, activity);
        }
        if (second.getBitmap() == null && sessionState.getRightImageUri() != null) {
            loadHolder(cr, sessionState.getRightImageUri(), second,
                    cacheDir, CacheManager.COMPARE_FILE_TWO, activity);
        }
    ...
    /**
     * Re-applies already-loaded bitmaps to the preview image views.
     * Call this after every configuration change when bitmaps are already in memory.
     */
    public static void restoreImageViews(ImageSessionState sessionState, ActivityMainBinding binding) {
        updateViews(sessionState, binding);
    }

    /**
     * Strips the {@code "1_"} / {@code "2_"} slot prefix that is prepended when a file is
     * copied into the cache, so that only the original filename is shown to the user.
     */
    public static String stripSlotPrefix(String name) {
        if (name != null && name.length() > 2
                && (name.startsWith("1_") || name.startsWith("2_"))) {
