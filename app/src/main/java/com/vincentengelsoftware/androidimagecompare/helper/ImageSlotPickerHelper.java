...
/**
 * Registers and wires the gallery-picker and camera {@link ActivityResultLauncher}s for a
 * single image slot ({@code "left"} or {@code "right"}).
 * <p>
 * <strong>Lifecycle requirement:</strong> call {@link #create} in {@code Activity.onCreate}
 * before the activity reaches the STARTED state, because
 * {@link ActivityResultLauncher}s must be registered before that point.
 * <p>
 * The helper also handles the three-option image-source dialog (camera / gallery / share)
 * that appears when the user taps an image preview.
 */
public class ImageSlotPickerHelper {
    ...
    private ImageSlotPickerHelper(
        ...
        this.galleryPicker = activity.registerForActivityResult(
                new ActivityResultContracts.GetContent(), this::onGalleryResult);
        this.cameraPicker  = activity.registerForActivityResult(
    ...
    /**
     * Returns the {@link View.OnClickListener} to attach to the image preview view.
     * Tapping shows a dialog offering camera, gallery, or share actions.
     */
    ...
    private void onCameraResult(boolean success) {
        ...
        try {
            ...
            File destFile = new File(activity.getCacheDir(),
                    "IMG_" + System.currentTimeMillis() + ".jpg");
            Uri  destUri  = ImageFileSaver.saveUriToFile(
            ...
            setUri(destUri);
            getHolder().updateFromBitmap(
                    BitmapExtractor.fromUri(activity.getContentResolver(), destUri),
                    Dimensions.maxSide,
    ...
    private void onGalleryResult(Uri receivedUri) {
        ...
        Uri    localUri     = ImageFileSaver.saveUriToFile(
                activity.getContentResolver(), receivedUri, localFile);
        ...
        holder.updateImageViewPreviewImage(
                isLeft() ? binding.homeImageLeft : binding.homeImageRight);
        (isLeft() ? binding.mainTextViewNameImageLeft
                  : binding.mainTextViewNameImageRight).setText(holder.getImageName());
    }
        ...
        try {
            ...
            if (imageUri == null) {
                Toast.makeText(activity,
                        activity.getString(R.string.error_msg_missing_images),
            ...
            Toast.makeText(activity,
                    activity.getString(R.string.error_message_general),
                    Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private boolean isLeft() { return "left".equals(slot); }
