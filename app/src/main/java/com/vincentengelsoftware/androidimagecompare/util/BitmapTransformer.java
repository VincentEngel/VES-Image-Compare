...
public class BitmapTransformer {
  ...
  /**
   * Reused across calls – configured once, never mutated again, so it is safe to share between
   * calls (Canvas reads paint properties but does not write them).
   */
  ...
  public static Bitmap getCutBitmapWithTransparentBackgroundWithCanvas(
      Bitmap bitmap, Bitmap transparentBitmap, int width, boolean cutFromRightToLeft) {
    Bitmap cutBitmap = BitmapTransformer.getCutBitmap(bitmap, width, cutFromRightToLeft);

    if (cutFromRightToLeft) {
      width = 0;
    }

    return BitmapTransformer.mergeBitmap(cutBitmap, transparentBitmap, width);
  }
  ...
  private static Bitmap getCutBitmapFromPoints(Bitmap bitmapSource, Point[] points) {
