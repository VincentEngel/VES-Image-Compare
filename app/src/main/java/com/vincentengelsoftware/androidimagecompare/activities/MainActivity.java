...
import com.vincentengelsoftware.androidimagecompare.globals.DimensionsInitializer;
...
import com.vincentengelsoftware.androidimagecompare.helper.CompareModeDialogHelper;
import com.vincentengelsoftware.androidimagecompare.helper.ImageRestoreHelper;
import com.vincentengelsoftware.androidimagecompare.helper.ImageSlotPickerHelper;
import com.vincentengelsoftware.androidimagecompare.helper.IntentImageHandler;
import com.vincentengelsoftware.androidimagecompare.helper.MainHelper;
import com.vincentengelsoftware.androidimagecompare.helper.ResizeImageDialogHelper;
import com.vincentengelsoftware.androidimagecompare.services.CacheManager;
 ...
 * <p>
 * Responsibilities are intentionally kept to a minimum:
 * <ul>
 *   <li>Android lifecycle hooks ({@code onCreate}, {@code onResume}, …).</li>
 *   <li>View-binding setup and toolbar / button wiring ({@link #setUpActions}).</li>
 *   <li>Delegation to focused single-responsibility helpers for every non-trivial task.</li>
 * </ul>
 *
 * <p>Non-Activity concerns are handled by:
 * <ul>
 *   <li>{@link ImageSessionState}        — left/right URI + ImageInfoHolder singleton state</li>
 *   <li>{@link DimensionsInitializer}    — one-time screen-size calculation</li>
 *   <li>{@link CacheManager}             — cache file names + cleanup on destroy</li>
 *   <li>{@link ImageRestoreHelper}       — bitmap reload + image-view update after recreation</li>
 *   <li>{@link IntentImageHandler}       — incoming share-intent parsing</li>
 *   <li>{@link ResizeImageDialogHelper}  — resize options dialog</li>
 *   <li>{@link CompareModeDialogHelper}  — compare-mode selection dialog</li>
 *   <li>{@link CompareActivityLauncher}  — parallel image encoding + activity launch</li>
 *   <li>{@link ImageSlotPickerHelper}    — camera / gallery / share per image slot</li>
 * </ul>
 */
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ApplyUserSettings.apply(this.userSettings,
        ...
        if (savedInstanceState != null) {
            ...
            if (firstWasNull && sessionState.getFirstImageInfoHolder().getBitmap() != null) {
                sessionState.getFirstImageInfoHolder().restoreTransformState(
                        savedInstanceState.getBundle("leftImageTransformState"));
                sessionState.getFirstImageInfoHolder()
                        .updateImageViewPreviewImage(binding.homeImageLeft);
            }
            if (secondWasNull && sessionState.getSecondImageInfoHolder().getBitmap() != null) {
                sessionState.getSecondImageInfoHolder().restoreTransformState(
                        savedInstanceState.getBundle("rightImageTransformState"));
                sessionState.getSecondImageInfoHolder()
                        .updateImageViewPreviewImage(binding.homeImageRight);
            }
        }
    ...
    protected void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        if (sessionState.getLeftImageUri() != null) {
            outState.putString(ImageSessionState.LEFT_URI_KEY,
                    sessionState.getLeftImageUri().toString());
        }
        if (sessionState.getRightImageUri() != null) {
            outState.putString(ImageSessionState.RIGHT_URI_KEY,
                    sessionState.getRightImageUri().toString());
        }
        if (sessionState.getFirstImageInfoHolder().getBitmap() != null) {
            outState.putBundle("leftImageTransformState",
                    sessionState.getFirstImageInfoHolder().saveTransformState());
        }
        if (sessionState.getSecondImageInfoHolder().getBitmap() != null) {
            outState.putBundle("rightImageTransformState",
                    sessionState.getSecondImageInfoHolder().saveTransformState());
        }
    }
    ...
    protected void onResume() {
        ...
        binding.homeButtonExtensions.setImageDrawable(ContextCompat.getDrawable(getBaseContext(),
                this.userSettings.isShowExtensions()
                        ? R.drawable.ic_extension_on
                        : R.drawable.ic_extension_off));

        binding.homeButtonLinkZoom.setImageDrawable(ContextCompat.getDrawable(getBaseContext(),
    ...
    private void setUpActions() {
        ...
        binding.mainBtnResizeImageLeft.setOnClickListener(view ->
                ResizeImageDialogHelper.show(this,
                        sessionState.getFirstImageInfoHolder(),
        ...
        binding.mainButtonLastCompare.setOnClickListener(view -> {
            String mode = CompareModeNames.getInternalCompareModeNameFromUserCompareModeName(
        ...
        binding.homeButtonExtensions.setOnClickListener(view -> {
            ...
            binding.homeButtonExtensions.setImageDrawable(ContextCompat.getDrawable(
                    getBaseContext(),
                    this.userSettings.isShowExtensions()
                            ? R.drawable.ic_extension_on
                            : R.drawable.ic_extension_off));
        ...
        binding.homeButtonExtensions.setOnLongClickListener(view -> {
            Toast.makeText(this, R.string.show_extensions_in_compare_modes, Toast.LENGTH_SHORT).show();
            return true;
        });
        ...
        binding.homeButtonLinkZoom.setOnClickListener(view -> {
            ...
            binding.homeButtonLinkZoom.setImageDrawable(ContextCompat.getDrawable(
                    getBaseContext(),
                    this.userSettings.isSyncImageInteractions()
                            ? R.drawable.ic_link
                            : R.drawable.ic_link_off));
        ...
        binding.homeButtonLinkZoom.setOnLongClickListener(view -> {
            Toast.makeText(this, R.string.globally_enable_or_disable_linked_zoom, Toast.LENGTH_SHORT).show();
            return true;
        });
        ...
        MainHelper.addRotateImageLogic(
                ...
                sessionState.getFirstImageInfoHolder(),
                binding.homeImageLeft, openingActivity);
        ...
        MainHelper.addRotateImageLogic(
                ...
                sessionState.getSecondImageInfoHolder(),
                binding.homeImageRight, openingActivity);
        ...
        leftPicker  = ImageSlotPickerHelper.create(
                this, "left",  sessionState, binding, openingActivity);
        rightPicker = ImageSlotPickerHelper.create(
                this, "right", sessionState, binding, openingActivity);
