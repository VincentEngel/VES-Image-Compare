 ...
 *
 * <p>The controls bar slides in when the user interacts with the screen and automatically slides
 * out after {@link ResizeAnimation#DURATION_LONG} ms of inactivity.
 *
 * <p>Lifecycle: create in {@code Activity.onCreate()}, call {@link #destroy()} from {@code
 * Activity.onDestroy()} to cancel any pending callbacks and prevent leaks.
 */
public class ControlsBarManager {
  ...
  /**
   * Animates the controls bar back into view, then schedules the next auto-hide. No-op if a show
  ...
  /**
   * Immediately snaps the controls bar to full height without animation, then schedules the next
   * auto-hide.
   */
  ...
  /**
   * Posts a delayed runnable that slides the controls bar out of view. Cancels and replaces any
   * previously pending hide.
   */
  ...
  /**
   * Cancels any pending auto-hide runnable. Safe to call at any time, including after {@link
   * #destroy()}.
   */
  ...
  /**
   * Cancels pending callbacks to prevent Handler leaks. Must be called from the host Activity's
   * {@code onDestroy()}.
   */
  public void destroy() {
    cancelPendingHide();
