...
import com.vincentengelsoftware.androidimagecompare.data.preferences.ImageResizeSettings;
 ...
 *
 * <p>Use the single static entry point {@link #show(Activity, ImageInfoHolder,
 * ImageResizeSettings)}.
 */
public class ResizeImageDialogHelper {
  ...
  public static void show(
      Activity activity, ImageInfoHolder imageInfoHolder, ImageResizeSettings imageResizeSettings) {
    ...
    dialogBinding.dialogResizeImageRadioGroup.setOnCheckedChangeListener(
        (group, checkedId) -> {
          ...
          dialogBinding.dialogResizeImageOriginalInfoText.setVisibility(
              isOriginal ? View.VISIBLE : View.GONE);
          dialogBinding.dialogResizeImageAutomaticInfoText.setVisibility(
              isAutomatic ? View.VISIBLE : View.GONE);
          dialogBinding.dialogResizeImageCustomSettings.setVisibility(
              isCustom ? View.VISIBLE : View.GONE);
        });
    ...
    dialogBinding.dialogResizeImageBtnDone.setOnClickListener(
        v -> {
          ...
          if (checkedId == R.id.dialog_resize_image_radio_button_original) {
            ...
            try {
              height =
                  Integer.parseInt(dialogBinding.dialogResizeImageInputHeight.getText().toString());
              width =
                  Integer.parseInt(dialogBinding.dialogResizeImageInputWidth.getText().toString());
            } catch (Exception e) {
              Toast.makeText(
                      activity.getApplicationContext(),
                      R.string.error_msg_invalid_input_not_a_number,
                      Toast.LENGTH_SHORT)
                  .show();
              return;
            }
            ...
            if (height <= 0 || width <= 0) {
              Toast.makeText(
                      activity.getApplicationContext(),
                      R.string.error_msg_invalid_input_number_gt_zero,
                      Toast.LENGTH_SHORT)
                  .show();
              return;
            }
