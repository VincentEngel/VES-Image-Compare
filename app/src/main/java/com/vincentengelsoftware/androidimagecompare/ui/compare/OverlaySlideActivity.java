...
/**
 * Displays two images stacked on top of each other and lets the user slide a vertical divider left
 * or right to reveal either image.
 *
...
public class OverlaySlideActivity extends AppCompatActivity implements ControlsBarHost {
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if (savedInstanceState != null) {
      viewModel.getSync().set(savedInstanceState.getBoolean(KEY_SYNC, true));
      viewModel.getLeftToRight().set(savedInstanceState.getBoolean(KEY_LEFT_TO_RIGHT, true));
    } else {
      viewModel
          .getSync()
  ...
  /**
   * Observes the ViewModel's crop output. Each new bitmap is applied to the front image view while
   * preserving the current zoom/pan state.
   */
  ...
  private void initControls() {
    ...
    binding.overlaySlideSeekBar.setOnSeekBarChangeListener(
  ...
  /**
   * Hides the front image at the edges of the seekbar range, or submits a crop job to the ViewModel
  ...
  private void updateSlideDirectionIcon() {
    binding.overlaySlideButtonSwapSeekbar.setImageResource(
        viewModel.getLeftToRight().get() ? R.drawable.ic_slide_ltr : R.drawable.ic_slide_rtl);
  }
}
