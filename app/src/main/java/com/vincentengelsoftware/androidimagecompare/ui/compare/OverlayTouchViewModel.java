 ...
 *
 * <p>Owns the front-image bitmap so that the erased state is preserved across rotations.
 * The bitmap lifecycle is managed here: it is recycled in {@link #onCleared()} when the
 * Activity is permanently finished, never when it is merely re-created.
 *
 * <p>Also persists the two UI states that Android's default view-state saving does not cover:
 * <ul>
 *   <li>Erase toggle (enabled / paused)</li>
 *   <li>Brush-size seekbar progress</li>
 * </ul>
 */
public class OverlayTouchViewModel extends ViewModel {
    ...
    /**
     * Mutable copy of the front image; transparent holes are punched here by
     * {@link TouchRevealView}.  {@code null} until the background decode finishes.
     */
    @Nullable private Bitmap mutableBitmap;

    /**
     * Compact JPEG snapshot of the original front image used by
     * {@link TouchRevealView#reset()}.
     */
    ...
    /**
     * Last seekbar progress value (range 1–200).
     * Default matches {@code android:progress="60"} in the layout XML.
     */
    ...
    /**
     * Returns {@code true} if a valid (non-recycled) bitmap is already held.
     * Used by the Activity to decide whether to re-decode or restore directly.
     */
    ...
    /**
     * Stores the decoded bitmap and its JPEG snapshot.
     * Must be called from the background decode thread before posting to the UI thread,
     * so that the ViewModel already holds the data if the Activity is destroyed mid-flight.
     */
