...
/**
 * Displays two images stacked on top of each other and lets the user "paint away" the
 * front image by touching the screen, revealing the background image underneath.
 *
 * <p>The background image is shown inside a {@link com.vincentengelsoftware.androidimagecompare.ui.widget.ZoomImageView}
 * so the user can still zoom/pan it. The front image is rendered by a {@link TouchRevealView}
 * which intercepts touch events and erases circular areas at each touch point.
 *
 * <p>All UI state (erasing enabled, brush size) and the erased bitmap itself are retained
 * across configuration changes via {@link OverlayTouchViewModel}, so rotating the screen
 * preserves the current erase marks, seekbar position, and pause/resume status.
 *
 * <p>Controls (shown when {@code SHOW_EXTENSIONS} is {@code true}):
 * <ul>
 *   <li>Reset button – restores the front image to its original state.</li>
 *   <li>Brush-size seekbar – adjusts the erase radius in real time.</li>
 * </ul>
 */
public class OverlayTouchActivity extends AppCompatActivity {
    ...
    /**
     * Reads the two image URIs from the Intent, sets the background image on the
     * {@link com.vincentengelsoftware.androidimagecompare.ui.widget.ZoomImageView} and
     * either restores the front image from the ViewModel (after a configuration change)
     * or kicks off a background thread to decode it for the first time.
     *
     * <p>Storing the bitmap in the ViewModel <em>before</em> posting to the UI thread
     * ensures it is available even if the Activity is destroyed mid-flight, so the next
     * recreation can restore it immediately without re-decoding.
     *
    ...
    private void initControls() {
        ...
        binding.overlayTouchButtonEraseToggle.setOnClickListener(v -> {
        ...
        binding.overlayTouchButtonReset.setOnClickListener(v ->
                binding.overlayTouchRevealView.reset());
