...
/**
 * Handles incoming {@link Intent#ACTION_SEND} and {@link Intent#ACTION_SEND_MULTIPLE}
 * share intents that deliver images to this app.
 * <p>
 * The first shared image always fills the left slot; the second fills the right slot.
 * Sharing more than two images shows a toast and discards the extras.
 */
public class IntentImageHandler {
    ...
    private static void handleSendImage(
        ...
        ImageInfoHolder holder = isFirst
        ...
        Uri    localUri     = ImageFileSaver.saveUriToFile(
                activity.getContentResolver(), imageUri, localFile);
    ...
    private static void handleSendMultipleImages(
        ...
        if (!imageUris.isEmpty() && imageUris.get(0) != null) {
            ...
            if (localUri != null) {
                ...
                MainHelper.updateImageFromIntent(
                        sessionState.getFirstImageInfoHolder(),
                        BitmapExtractor.fromUri(activity.getContentResolver(), localUri),
                        Dimensions.maxSide, Dimensions.maxSideForPreview,
                        name, binding.homeImageLeft, binding.mainTextViewNameImageLeft
                );
            }
        }
        ...
        if (imageUris.size() > 1 && imageUris.get(1) != null) {
            ...
            if (localUri != null) {
                ...
                MainHelper.updateImageFromIntent(
                        ...
                        BitmapExtractor.fromUri(activity.getContentResolver(), localUri),
                        Dimensions.maxSide, Dimensions.maxSideForPreview,
                        name, binding.homeImageRight, binding.mainTextViewNameImageRight
                );
            }
        }
        ...
        if (imageUris.size() > 2) {
            Toast.makeText(activity.getApplicationContext(),
                    R.string.error_message_intent_more_than_two_images, Toast.LENGTH_LONG).show();
        }
    }
}
