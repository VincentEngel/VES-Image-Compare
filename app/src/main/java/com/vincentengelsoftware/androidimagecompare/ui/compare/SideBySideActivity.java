 ...
 *
 * <p>The Activity owns only UI concerns: view binding, controls wiring, and
 * LiveData observation.  All bitmap loading and state management is delegated
 * to {@link SideBySideViewModel}.</p>
 */
public class SideBySideActivity extends AppCompatActivity {
    ...
    /**
     * Starts observing the ViewModel's LiveData streams.  Must be called after
     * the binding is ready so that view updates are always on the main thread.
     */
    private void observeViewModel() {
        viewModel.getImages().observe(this, imagePair -> {
        ...
        viewModel.isLoadFailed().observe(this, failed -> {
    ...
    /**
     * Wires synchronised zoom/pan between the two image views and binds the
     * toggle button that enables/disables the link.
     */
    ...
    /**
     * Asks the ViewModel to decode both bitmaps in the background.
     * This is a no-op after a configuration change (bitmaps are already retained).
     */
