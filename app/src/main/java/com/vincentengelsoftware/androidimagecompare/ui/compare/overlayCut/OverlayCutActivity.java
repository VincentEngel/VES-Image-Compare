...
/**
 * Displays two images stacked on top of each other and lets the user draw a diagonal cut-line by
...
public class OverlayCutActivity extends AppCompatActivity {
  ...
  /**
   * Tracks the two most-recently touched seekbars. Exactly two seekbars define the diagonal cut
   * line.
   */
  ...
  /**
   * Starts observing the ViewModel's live output. Must be called after the binding is ready so that
   * {@link android.widget.ImageView#setImageBitmap} is always invoked on the main thread.
   */
  ...
  /**
   * Loads the two images from the intent URIs (first launch) or reuses the bitmaps already retained
   * by the ViewModel (after a configuration change).
   *
  ...
  private void initSeekBars() {
    ...
    int seekBarLength =
        getResources().getDisplayMetrics().heightPixels - Calculator.dpToPx(48, getResources());
  ...
  private void initExtensionButtons() {
    ...
    binding.overlayCutBtnReset.setOnClickListener(
        view -> {
          ...
          if (seekBarTracker.canCrop() && viewModel.bitmapAdjusted != null) {
            viewModel.submitCrop(
                viewModel.bitmapAdjusted.copy(Bitmap.Config.ARGB_8888, true), buildCropParams());
          } else if (viewModel.bitmapAdjusted != null) {
            binding.fullSlideImageViewFront.setBitmapImage(viewModel.bitmapAdjusted);
          }
        });

    binding.overlayCutBtnCheck.setOnClickListener(
        view -> {
  ...
  /**
   * Reads the current seekbar positions and active-flags from {@link #seekBarTracker} to build an
   * immutable {@link CropParams} snapshot.
   */
  @NonNull
  ...
  /**
   * Tracks the two most-recently touched seekbars (current and recent). Exactly this pair defines
   * the active diagonal crop line.
   */
  private static final class SeekBarTracker {
     ...
     * @return the seekbar that was just evicted from the active pair and therefore needs its thumb
     *     dimmed, or {@code null} if nothing changed in the active pair
     */
    @Nullable
    SeekBar onSeekBarChanged(@NonNull SeekBar seekBar) {
      ...
      SeekBar evicted = (recent != null && seekBar.getId() != recent.getId()) ? recent : null;
      recent = current;
      current = seekBar;
      return evicted;
    }
    ...
    /**
     * Returns {@code true} when both {@link #current} and {@link #recent} are set, i.e. a crop
    ...
    /**
     * Returns {@code true} when {@code seekBar} is one of the two currently active seekbars
     * (current or recent).
     */
    boolean isActive(@NonNull SeekBar seekBar) {
      int id = seekBar.getId();
      return (current != null && id == current.getId()) || (recent != null && id == recent.getId());
    }
  }
}
