 ...
 *
 * <p>Provides factory methods that build {@link File} references from a cache directory, and a
...
public class CacheManager {
  ...
  public static void cleanup(File cacheDir, ImageSessionState sessionState) {
    try {
      ...
      Set<String> keepPaths = buildKeepPaths(cacheDir, sessionState);
      for (File file : files) {
        try {
  ...
  private static Set<String> buildKeepPaths(File cacheDir, ImageSessionState state)
      throws IOException {
    Set<String> paths = new HashSet<>();
    addUriPath(paths, state.getLeftImageUri());
    addUriPath(paths, state.getRightImageUri());

    // Always preserve the camera temp file and calculated image files
    paths.add(getCameraTempFile(cacheDir).getCanonicalPath());
    paths.add(getCompareFileOne(cacheDir).getCanonicalPath());
    paths.add(getCompareFileTwo(cacheDir).getCanonicalPath());
